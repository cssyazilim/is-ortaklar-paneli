(function(){
  ...
  function renderCompanyContent(){
    ...
    // Şirket ise, solda “İletişim Kişisi” kalmışsa kaldır.
    if (!isSahis) {
  ...
  async function loadPartnerProfile(){
    try{
      ...
      if (!res.ok){
        showMessage('Profil bilgileri alınamadı ('+res.status+').', 'error');
        return;
      }

      let data;
      try { data = JSON.parse(raw); }
      catch(e){ console.error('JSON parse hatası', e, raw); showMessage('Geçersiz yanıt alındı.', 'error'); return; }

      // Skeletonları gerçek içerikle değiştir
      restoreCover();
      restoreStatusCard();
      restoreSecurityCard();   // 🔥 burayı ekle

      // Güvenlik template’i DOM’a eklendiği için şimdi event’leri bağla
document.querySelectorAll('input[name="mfa_method"]').forEach(r =>
  ...
  // Defer ile yüklendiği için DOM hazır olur olmaz çalışır
  window.addEventListener('DOMContentLoaded', ()=>{
    loadPartnerProfile(); // Skeletonlar zaten PHP’den geliyor
    document.querySelectorAll('input[name="mfa_method"]').forEach(r =>
      r.addEventListener('change', ()=>updateMFAInput(true))
    );
  });
